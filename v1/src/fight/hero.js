...
function realHero(ch, url, clips){
  ...
  // Все клипы приводятся к полу ОДИН РАЗ при загрузке — поправка вшивается
  // прямо в дорожку таза. Раньше она применялась на лету, и при переходе
  // между клипами с разной высотой таза всё тело съезжало на полметра:
  // со стороны это выглядело так, будто боец отходит и возвращается.
  //
  // У прыжка высоту считает игра, поэтому подъём из клипа убирается целиком,
  // иначе подъёмы складываются и получается прыжок внутри прыжка.
  const refGround = ch.bindGround();
  if(ch.actions.jump) ch.flattenClip("jump");
  for(const name of [...Object.keys(LOOPS), ...Object.keys(MOVES)]){
